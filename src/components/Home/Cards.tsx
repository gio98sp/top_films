...
export const Cards = ({ category, subCategory }: ICardsProps) => {
  const [data, setData] = useState<IgetMoviesOrSeries>();
  const [page, setPage] = useState(1);

  useEffect(() => {
    getMoviesOrSeries({ category, subCategory, page })
  ...
  return (
    <div className="max-w-screen-xl m-auto mb-16 md:mb-24 px-5 sm:px-7 flex flex-col gap:16 md:gap-24">
      <TitleCards category={category} subCategory={subCategory} />
      ...
      <div className="flex flex-wrap justify-evenly items-center gap-5 gap-y-8 md:gap-12">
        {data?.results.map(
          (item: any, index: number) =>
            item.poster_path &&
            index < 18 && (
              <Link href={`/details/${item.id}/${category}`} key={item.id}>
      ...
      <Pagination changePagination={changePagination} />
    </div>
  );
};
